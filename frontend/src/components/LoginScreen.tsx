...
export default function LoginScreen({ onLogin }: { onLogin: (result: LoginResult) => void }) {
  ...
  async function handleSubmit() {
    ...
    try {
      const recaptchaToken = await getRecaptchaToken(mode === 'register' ? 'register' : 'login');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100dvh', background: 'radial-gradient(ellipse 60% 40% at 50% 60%, #00e5a018 0%, transparent 70%), var(--bg)' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', width: 'min(380px, 90vw)', padding: '2.5rem', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6 }}>
        ...
        <button onClick={handleSubmit} disabled={loading} style={{ padding: '0.8rem', background: 'var(--accent)', color: '#000', fontFamily: 'var(--display)', fontWeight: 700, fontSize: '0.9rem', letterSpacing: '0.05em', border: 'none', borderRadius: 6, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
          {loading ? 'Please wait…' : mode === 'register' ? 'Create account →' : 'Sign in →'}
        </button>

        {RECAPTCHA_SITE_KEY && (
          <div style={{ fontSize: '0.65rem', color: 'var(--text-dim)', textAlign: 'center' }}>
            Protected by reCAPTCHA — <a href="https://policies.google.com/privacy" target="_blank" rel="noopener" style={{ color: 'var(--text-dim)' }}>Privacy</a> · <a href="https://policies.google.com/terms" target="_blank" rel="noopener" style={{ color: 'var(--text-dim)' }}>Terms</a>
          </div>
        )}
      </div>
    </div>
