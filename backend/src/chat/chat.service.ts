...
const MAX_HISTORY_LOAD = 200; // max messages returned per room on connect
...
export class ChatService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    // Prune old messages on startup (and schedule periodic pruning)
    this.pruneOldMessages();
    setInterval(() => this.pruneOldMessages(), 60 * 60 * 1000); // every hour
  ...
  private pruneOldMessages() {
    ...
    if (result.changes > 0) console.log(`[chat] Pruned ${result.changes} messages older than 30 days`);
  }
